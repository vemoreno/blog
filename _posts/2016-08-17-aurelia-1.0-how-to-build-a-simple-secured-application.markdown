...
With Aurelia 1.0, we can easily define the plugins and resources we want to load in either `debug`, `testing`, or `production` mode. Now, let's create the `auth-config` file:
...
export class RandomQuote {
  ...
  constructor(http) {
    this.http = http; this.http = http.configure(x => {
...
As you can see, we are configuring the http client to send an authorization header that carries the id_tokenthe only real difference here is that the GET request we're making is going to the `protected/random-quote` endpoint. If there is no valid JWT in `localStorage`, we won't be able to get to this route. If somehow we got to it, the request would fail because no JWT would be sent to the server.
